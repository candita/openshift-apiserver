...
func TestValidate(t *testing.T) {
	...
	for _, tc := range tests {
		routeLister := fakeRouteLister(map[string]map[string]string{"route1": {"default": ""}, "route2": {"foo": "bar"}, "route3": {"abc.foo.com": "bar"}}, tc.spec)
		ingressLister := fakeIngressLister("cluster", *tc.config)
...
func fakeRouteLister(routeDetails map[string]map[string]string, spec *routeapi.RouteSpec) routev1listers.RouteLister {
	indexer := cache.NewIndexer(cache.MetaNamespaceKeyFunc, cache.Indexers{})
	for route, namespaceAndLabels := range routeDetails {
		for ns := range namespaceAndLabels {
			indexer.Add(fakeRoute(route, ns, spec, namespaceAndLabels))
		}
	}
